...
${sourceText}`;
    ...
    // Stage 1: Extract company snapshot from provided text
...
If a field is not mentioned in the source, write "Not mentioned in sources".` }],
    ...
    // Stage 2: Extract management claims
      ...
      {expanded && (
        <div style={{ marginTop: 8, padding: "10px 12px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", fontSize: 12, color: "var(--color-text-secondary)", lineHeight: 1.6 }}>
          ...
          {flag.requires_human_review && (
            <div style={{ marginTop: 8, padding: "6px 10px", background: "#FAEEDA", borderRadius: "var(--border-radius-md)", color: "#854F0B", fontSize: 11 }}>
...
function PipelineProgress({ stage, progress }) {
  return (
    <div style={{ padding: "1.5rem 0" }}>
      ...
      {STAGES.map((s, i) => (
        <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "7px 0", opacity: i > stage ? 0.3 : 1 }}>
          <div style={{ width: 26, height: 26, borderRadius: "50%", background: i < stage ? "#EAF3DE" : i === stage ? "#FCEBEB" : "var(--color-background-secondary)", border: `0.5px solid ${i < stage ? "#639922" : i === stage ? "#E24B4A" : "var(--color-border-tertiary)"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
            {i < stage
              ? <span style={{ fontSize: 12, color: "#3B6D11" }}>✓</span>
              : <i className={`ti ${s.icon}`} style={{ fontSize: 12, color: i === stage ? "#A32D2D" : "var(--color-text-secondary)" }} aria-hidden="true" />
            }
          </div>
          <span style={{ fontSize: 13, color: i === stage ? "var(--color-text-primary)" : "var(--color-text-secondary)", fontWeight: i === stage ? 500 : 400 }}>
            {s.label}
            {i === stage && <span style={{ marginLeft: 8, fontSize: 11, color: "#A32D2D" }}>● running</span>}
          </span>
        </div>
      ))}
    </div>
...
function ReportView({ report, onReset }) {
  const { company_info: co, core_thesis, red_flags, contradictions, report_sections: rs } = report;
    ...
    <div style={{ fontFamily: "var(--font-sans)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: "0.5px solid var(--color-border-tertiary)", marginBottom: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <p style={{ margin: 0, fontWeight: 500, fontSize: 16, color: "var(--color-text-primary)" }}>{co.company_name} {co.ticker !== "N/A" ? `(${co.ticker})` : ""}</p>
            <p style={{ margin: 0, fontSize: 12, color: "var(--color-text-secondary)" }}>Investigative report · {new Date(report.generated_at).toLocaleDateString()}</p>
          </div>
        </div>
      ...
      <div style={{ minHeight: 300 }}>
        {activeSection === "snapshot" && (
          <div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
              {[["Company", co.company_name], ["Ticker", co.ticker], ["Sector", co.sector], ["CEO", co.ceo], ["CFO", co.cfo], ["Auditor", co.auditor], ["Exchange", co.exchange], ["Revenue", co.revenue], ["Market cap", co.market_cap]].map(([k,v]) => (
            ...
            <div style={{ padding: "12px 16px", background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-lg)", fontSize: 13, color: "var(--color-text-secondary)", lineHeight: 1.7 }}>
              {co.description}
            </div>
            {co.recent_controversies?.length > 0 && (
              <div style={{ marginTop: 16 }}>
                <p style={{ fontSize: 11, fontWeight: 500, color: "var(--color-text-secondary)", marginBottom: 8, textTransform: "uppercase" }}>Recent controversies</p>
        ...
        {activeSection === "thesis" && (
          <div>
            <div style={{ padding: "16px 20px", background: "#FCEBEB", border: "0.5px solid #E24B4A", borderLeft: "4px solid #E24B4A", borderRadius: "var(--border-radius-lg)", marginBottom: 16 }}>
              ...
              <p style={{ margin: 0, fontSize: 14, lineHeight: 1.8, color: "#501313" }}>{core_thesis}</p>
            </div>
          ...
          <div>
            {contradictions.length === 0
              ...
              : contradictions.map((c,i) => (
                <div key={i} style={{ marginBottom: 14, background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-tertiary)", borderRadius: "var(--border-radius-lg)", overflow: "hidden" }}>
        ...
        {activeSection === "risks" && (
          <div>
            <div style={{ padding: "16px 20px", background: "#FAEEDA", border: "0.5px solid #EF9F27", borderLeft: "4px solid #EF9F27", borderRadius: "var(--border-radius-lg)", marginBottom: 16, fontSize: 14, color: "#633806", lineHeight: 1.8 }}>
        ...
        {activeSection === "conclusion" && (
          <div>
            <div style={{ padding: "16px 20px", background: "var(--color-background-primary)", border: "0.5px solid var(--color-border-secondary)", borderLeft: "4px solid #E24B4A", borderRadius: "var(--border-radius-lg)", marginBottom: 16, fontSize: 14, color: "var(--color-text-primary)", lineHeight: 1.8 }}>
              {rs.conclusion}
            </div>
...
export default function App() {
  ...
  if (report) {
    return <ReportView report={report} onReset={() => { setReport(null); setCompanyName(""); setTicker(""); setSourceText(""); }} />;
  }
        ...
        {!apiKeySaved ? (
          <>
            <p style={{ margin: "0 0 10px", fontSize: 12, color: "var(--color-text-secondary)" }}>
              Get a free key at <strong>console.anthropic.com</strong> → sign up → API Keys → Create Key
            </p>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              />
              <button
                onClick={() => { if (apiKey.startsWith("sk-")) setApiKeySaved(true); }}
                disabled={!apiKey.startsWith("sk-")}
                style={{ padding: "0 16px", fontSize: 13, background: apiKey.startsWith("sk-") ? "#EAF3DE" : "var(--color-background-secondary)", border: `0.5px solid ${apiKey.startsWith("sk-") ? "#639922" : "var(--color-border-tertiary)"}`, color: apiKey.startsWith("sk-") ? "#3B6D11" : "var(--color-text-secondary)", borderRadius: "var(--border-radius-md)", cursor: "pointer" }}>
                Save key
              </button>
            </div>
          </>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 13, color: "#3B6D11" }}>✓ API key saved — {apiKey.slice(0,10)}...</span>
...
The more text you provide, the deeper the analysis.`}
        ...
        <p style={{ margin: "4px 0 0", fontSize: 11, color: "var(--color-text-secondary)" }}>
          {sourceText.length.toLocaleString()} characters · {Math.round(sourceText.split(/\s+/).filter(Boolean).length / 750)} min read
        </p>
      </div>
      ...
      {!running && (
        <button
          onClick={handleRun}
          disabled={!apiKeySaved || !companyName.trim() || !sourceText.trim()}
          style={{
            width: "100%", padding: "12px", fontSize: 14, fontWeight: 500,
            background: (apiKeySaved && companyName.trim() && sourceText.trim()) ? "#FCEBEB" : "var(--color-background-secondary)",
            border: `0.5px solid ${(apiKeySaved && companyName.trim() && sourceText.trim()) ? "#E24B4A" : "var(--color-border-tertiary)"}`,
            color: (apiKeySaved && companyName.trim() && sourceText.trim()) ? "#A32D2D" : "var(--color-text-secondary)",
            borderRadius: "var(--border-radius-lg)", cursor: (apiKeySaved && companyName.trim() && sourceText.trim()) ? "pointer" : "not-allowed",
          }}
        >
